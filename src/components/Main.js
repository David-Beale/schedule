...
function AboutPage (props) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (i < 23) localHours.push(<div id={`margin${i}`} className="grid-item1" key={i}>{`${index}:00`}</div>)
      else localHours.push(<div className="grid-item1 end" key={i}>{`${index}:00`}</div>)
    }
    setHours(localHours)
    const tempRows = []
    for (let j = 0; j < rowCounter; j++) {
      ...
      tempRows.push(<div className="flex-container fc2" id={`row${j}`} key={j}>{content}</div>)
    }
    setRows(tempRows)
  }, [])
  ...
  useEffect(() => {
    if (hours.length > 0) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        newContent.style.width = `${contentLengthHours * unitWidth + (contentLengthMinutes / 60) * unitWidth}px`
        ...
        document.querySelector(`#row${selectedRow} #time${contentHours}`).appendChild(newContent);
      }
